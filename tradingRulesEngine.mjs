...
const previousDayLiquidity = detectPreviousDayLiquidityConfirmation(this);
const dowStructure = detectDowStructureConfirmation(this);
  // ================================================================
...
// ÉTAPE 1C — DIRECTION ET LIQUIDITÉ JOURNALIÈRE

if (dowStructure.confirme) {
  report.validation.push(
    `📈 Direction : ${dowStructure.direction} — ${dowStructure.setupIdentifie}.`
  );
  report.validation.push(...dowStructure.details);
} else {
  report.validation.push(
    "📊 Direction : aucune structure HH/HL ou LH/LL clairement confirmée."
  );
  report.validation.push(...dowStructure.details);
}

if (previousDayLiquidity.confirme) {
  report.liquidite.push(
    `💧 Confirmation PDH/PDL : ${previousDayLiquidity.setupIdentifie}.`
  );
  report.liquidite.push(...previousDayLiquidity.details);
} else {
  report.liquidite.push(
    "💧 PDH/PDL : aucune réaction de liquidité confirmée."
  );
  report.liquidite.push(...previousDayLiquidity.details);
}
