...
import { auditerTechniquesSource, formatTechniquesSource } from "./arbitrageAuditTechniquesSource.js";
import {
  TRADING_MODE,
  TRADING_MODES,
  ALLOWED_SYMBOLS,
...
/* ============================================================
   AUDIT DU MOTEUR — lecture seule, aucun ordre, ne modifie rien
   Vérifie que chaque stratégie de tradingRulesEngine.js dispose bien de
   toutes les conditions nécessaires (voir arbitrageAuditMoteur.js).
   N'appelle jamais le broker, ne recalcule rien : lit uniquement le
   dernier rapport d'arbitrage déjà mis en cache pour le symbole/timeframe
   demandé (le même rapport que celui utilisé par le pipeline TradingView).
   ============================================================ */

app.get("/api/audit-moteur", (req, res) => {
  const symbol = req.query.symbol
    ? String(req.query.symbol).trim().toUpperCase()
    : derniereDecision?.symbol ?? null;
  const timeframe = req.query.timeframe
    ? String(req.query.timeframe).trim()
    : derniereDecision?.timeframe ?? null;
  ...
  if (!symbol || !timeframe) {
    return res.json({
      ...
      audit: null,
      message:
        "Aucune décision disponible pour déterminer un symbole/timeframe par défaut. " +
        "Précise ?symbol=...&timeframe=... ou attends une première alerte TradingView."
    });
  }

  const entreeCache = rapportsParSymboleEtTimeframe.get(symbol)?.[timeframe];
  ...
  if (!entreeCache) {
    return res.json({
      ...
      timeframe,
      message:
        `Aucun rapport d'arbitrage en cache pour ${symbol} (${timeframe}). ` +
        "Une alerte TradingView pour ce symbole/timeframe doit d'abord être reçue."
    });
  }

  const resultat = auditerMoteur(entreeCache.rapport);

  return res.json({
    ok: true,
    symbol,
    timeframe,
    recordedAt: new Date(entreeCache.recordedAt).toISOString(),
    verdictMoteur: entreeCache.rapport.verdict, // rappel : jamais modifié par l'audit
    resume: resultat.resume,
    audits: resultat.audits,
    texte: formatAuditMoteur(resultat)
  });
});


/* ============================================================
   AUDIT DES TECHNIQUES SOURCE — lecture seule, catalogue statique
   Dossier vidéo/photo (voir arbitrageAuditTechniquesSource.js). Ne dépend
   d'aucune décision ni d'aucun symbole : c'est une référence fixe.
   ============================================================ */

app.get("/api/audit-techniques-source", (req, res) => {
  const techniques = auditerTechniquesSource();
  return res.json({
    ok: true,
    total: techniques.length,
    techniques,
    texte: formatTechniquesSource()
  });
});
